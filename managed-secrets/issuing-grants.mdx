...
info = await app.agents.get_by_name("billing-bot")
billing_bot = app.get_agent(info.id)
...
const result2 = await agent.createManagedSecretGrant("ms_abc123", {
...
```python
# Inside agent code
result = await agent.create_managed_secret_grant(
    managed_secret_id="ms_abc123",
...
```python
page = await app.list_grants(limit=100)
