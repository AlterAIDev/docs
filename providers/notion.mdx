...
| **PKCE** | Not supported |
| **Token refresh** | Not used (long-lived access token) |
| **Redirect URI** | Shown in [Developer Portal](https://portal.alterauth.com) |

<Note>
  Notion permissions are granted per-page during authorization. This integration uses long-lived access tokens; Notion also supports refresh tokens if needed in the future.
</Note>
...
<Steps>
  ...
  <Step title="Add the redirect URI">
    Under **OAuth Domain & URIs**, add the **Redirect URI** from the [Developer Portal](https://portal.alterauth.com) to the **Redirect URIs** field.
  </Step>
...
- Notion supports refresh tokens, but this integration uses long-lived access tokens. Re-authorization is only needed if the user revokes access.
