...
Managed Secrets let **you** (the developer) store API keys, service tokens, and other credentials for services where you already have the credentials. No end-user OAuth flow is needed -- you store the secret in the Developer Portal and get back a `grant_id` to use with `vault.request()`.

Your code never sees the raw secret. You call `vault.request(grant_id, ...)` and the SDK injects the credential as the correct auth header automatically.
...
<Steps>
  ...
  <Step title="Get the grant_id">
    Each stored secret gets a `grant_id` (UUID). Save this in your code, config, or environment variables.
  </Step>
...
// grantId from Developer Portal (stored in your config)
const response = await vault.request(
  process.env.OPENAI_GRANT_ID!,
  HttpMethod.POST,
  "https://api.openai.com/v1/chat/completions",
  {
    json: { model: "gpt-4", messages: [{ role: "user", content: "Hello" }] },
