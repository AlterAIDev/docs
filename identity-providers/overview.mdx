...
This means when a user is **removed from your organization** in your IDP, their OAuth grants in Alter Vault are **immediately revoked** — no manual cleanup required.

<CardGroup cols={3}>
  <Card title="Automatic Deprovisioning" icon="shield-check">
    When users leave your org, their OAuth grants are immediately revoked and stored tokens deleted.
  </Card>
...
Beyond syncing identities, Alter Vault can use the IDP as a login provider. Configure OIDC client credentials in the Developer Portal to enable:

- **SDK authentication** --- `vault.authenticate()` opens a browser-based login flow through the IDP
- **Wallet dashboard sign-in** --- end users sign in to the Wallet dashboard with their IDP credentials

Setup requires registering two redirect URIs with the IDP and entering the OAuth client ID and secret in the Developer Portal. See the [IDP Setup Guide](/identity-providers/quickstart) for step-by-step instructions.
