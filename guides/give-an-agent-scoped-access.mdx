...
2. The operator binds credentials to the agent (a managed-secret grant directly, or a user [delegates](/concepts/credentials#delegation) an OAuth connection to the agent).
...
On Approve, Alter writes both the connection and a [delegation](/concepts/credentials#delegation) row binding it to the named agent.

**Path B — operator issues a managed secret to the agent.** From the Developer Portal: **Managed Secrets → [Provider] → New Grant → Agent → research-bot**. Agent-bound managed-secret grants are created from the Developer Portal flow; the SDK's `create_managed_secret_grant` / `createManagedSecretGrant` methods do not accept `AgentPrincipal`. The resulting `grant_id` is what the agent process uses in `request(grant_id=...)`.
...
<CardGroup cols={3}>
  <Card title="Delegation" icon="user-shield" href="/concepts/credentials#delegation">
