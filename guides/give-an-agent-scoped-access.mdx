...
**Path B — operator issues a managed secret to the agent.** From the Developer Portal: **Managed Secrets → [Provider] → New Grant → Agent → research-bot**. Agent-bound managed-secret grants are minted from the dev-portal admin route family — the SDK's `create_managed_secret_grant` / `createManagedSecretGrant` consumes a narrower principal union and rejects `{ "type": "agent" }` at the backend boundary. The resulting `grant_id` is what the agent process uses in `request(grant_id=...)`.
