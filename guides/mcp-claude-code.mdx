...
# Create the MCP auth provider. MCP clients discover the OAuth endpoints
# automatically via the server's discovery metadata.
...
<Note>
  The `base_url` parameter must match the public URL where the MCP server
  is mounted (e.g., `http://localhost:8000/mcp` in development). The SDK
  advertises OAuth endpoint URLs relative to it.
</Note>
