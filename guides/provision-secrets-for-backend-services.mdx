...
const alter = new App({ apiKey: process.env.ALTER_API_KEY! });

const result = await alter.createManagedSecretGrant("ms_datadog_abc", {
...
```typescript TypeScript
await alter.createManagedSecretGrant("ms_datadog_abc", {
  principal: {
    type: "group",
