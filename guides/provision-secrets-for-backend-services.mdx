...
2. The operator issues a grant binding the credential to a [principal](/concepts/credentials#principals) (system, user, group, or agent).
...
- **Group** — bind to an IDP group. All current and future members of the group can use it. Only available when the identity provider supports group grants (Clerk, or Okta with stable-ID group keying — see [the support table](/admin/identity-provider#how-directory-sync-works)).
...
<Note>
Group grants require an identity provider that delivers real-time group lifecycle webhooks — Clerk, or Okta with stable-ID group keying. Creating a group grant against any other provider (e.g. Auth0, Custom OIDC) is rejected. See the [per-provider support table](/admin/identity-provider#how-directory-sync-works).
</Note>
...
Any user whose JWT carries the `support` group claim can now resolve this grant under [JWT identity](/concepts/identity#identity-resolution).
