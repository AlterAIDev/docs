...
2. Alter Connect runs the OAuth flow and writes a [grant](/concepts/grants) bound to that user's identity.
...
app.post("/api/connect-session", authenticated, async (req, res) => {
...
app.get("/api/calendar/events", authenticated, async (req, res) => {
...
```typescript TypeScript
app.get("/api/connections", authenticated, async (req, res) => {
  await jwtStore.run(req.user.jwt, async () => {
    const page = await alter.listGrants();
    res.json(page.grants.map(g => ({
      provider: g.providerId, account: g.accountIdentifier, grantId: g.grantId,
    })));
  });
});
```
