...
A wrong `--confirm` value exits [2 (`USAGE`)](/cli/scripting#exit-codes); declining the interactive prompt exits 8 (`CANCELLED`).
...
- [`alter keys`](/cli/commands/keys), [`alter providers`](/cli/commands/providers), [`alter agents`](/cli/commands/agents) — the resources an app owns
- [Workspace linking](/cli/scripting#workspace-linking) — pin a default app so commands skip `--app`
