...
| `7` | FORBIDDEN | The PAT is valid but lacks the required scope. **Fix:** re-mint the PAT with broader [scopes](/cli/authentication#scopes). |
...
| `ALTER_API_KEY` | [`sdk-passthrough`](/cli/commands/sdk-passthrough) | Runtime API key for the one-off request escape hatch (not a PAT). |
