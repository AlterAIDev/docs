...
description: 'How Alter verifies end users from JWTs issued by Auth0, Clerk, Okta, Keycloak, WorkOS, or any OIDC provider.'
...
This page covers the IDP model. For the runtime SDK pattern, see [JWT identity](/oauth/jwt-identity). For step-by-step setup of a specific provider, see the per-IDP guides:
...
<CardGroup cols={3}>
  ...
  <Card title="Keycloak" icon="shield" href="/oauth/identity-providers/keycloak" />
  <Card title="WorkOS" icon="shield" href="/oauth/identity-providers/workos" />
