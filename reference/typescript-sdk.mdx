...
`request` and `proxyRequest` are the two [runtime modes](/concepts/runtime-modes). They share the same option shape; the difference is *where the third-party call happens* and *what the SDK returns*.
...
- `PendingApproval` — an approver must act before the call executes. Poll with `getApprovalStatus(approvalId)` or block with `awaitApproval(approvalId, { timeout })`. See the [HITL guide](/guides/add-human-in-the-loop-approvals).
