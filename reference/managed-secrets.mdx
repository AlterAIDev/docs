...
<Note>
  **OAuth vs Managed Secrets**: Use OAuth for third-party services where users authorize access (Google, Slack, GitHub). Use Managed Secrets for your own APIs where you already have the credentials (internal services, partner APIs, SaaS platforms with API keys).
</Note>
...
2. Click **Add Provider** to create a secret provider (e.g., "Loyalty API", "Billing Service")
...
4. Click **Store Secret** on the provider to save the credential value
...
| **Setup** | Alter Connect UI (frontend) | Developer Portal (backend) |
