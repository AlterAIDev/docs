...
<AccordionGroup>
  <Accordion title="Token & Secret Access" icon="key">
    Every time a credential is retrieved via `alter_app.request()` — both OAuth tokens and managed secrets. Includes provider, scopes, policy decision, actor, reason, and request intent (HTTP method and URL the caller plans to use).
  </Accordion>
...
- **Credential retrieved** — a managed secret was used via `alter_app.request()`
...
This makes it easy to investigate access patterns and debug policy violations. See the [Security Policies](/reference/policies) guide for configuring policies.

## Trace Context

Every audit log entry includes a `trace_id` field that correlates with the request's correlation ID. When a W3C `traceparent` header is sent, the trace ID from the distributed tracing system is captured automatically, linking audit entries directly to traces in the observability platform.
