...
<AccordionGroup>
  <Accordion title="Token Access" icon="key">
    Every time a token is retrieved via `vault.request()`. Includes provider, scopes, policy decision, actor, reason, and request intent (HTTP method and URL the caller plans to use).
  </Accordion>
