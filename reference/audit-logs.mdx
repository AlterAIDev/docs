...
In the Developer Portal, you can filter audit logs by caller to see exactly what each agent did and when. Registered callers also appear in the **Agents** tab.
...
The `context` dict must use string keys and string values. Oversized payloads raise `AlterValueError` at the SDK boundary.
...
<CardGroup cols={2}>
  <Card title="Agents" icon="robot" href="/agents/overview">
    Workload identities and audit attribution
  </Card>
