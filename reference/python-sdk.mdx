...
Same arg shape as `request`, but routes through the [HITL approval](/guides/add-human-in-the-loop-approvals) pipeline. Returns `PendingApproval` when an approval is required, `ApprovalResult` when the call ran synchronously.
