...
Mint a session for the user → agent [delegation](/concepts/delegation) flow on a managed secret. The end user signs in (verified against the app's IDP using `user_token`), reviews the agent identified by `delegated_agent_id`, and consents to delegate access on the `template_slug` managed secret.
