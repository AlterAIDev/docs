...
<CardGroup cols={3}>
  ...
  <Card title="Grant TTL" icon="hourglass">
...
<Steps>
  <Step title="Open the app in the Developer Portal">
    Go to [portal.alterauth.com](https://portal.alterauth.com) and select the app
  </Step>
...
const session = await alter_app.createConnectSession({
...
- When a grant expires (`expires_at < now`), any `alter_app.request()` call raises (Python) / throws (TypeScript) a `GrantExpiredError`
- End users can view and extend TTL in the [Wallet Dashboard](https://wallet.alterauth.com), up to the developer-set `max_ttl_seconds` bound
...
try {
  const response = await alter_app.request(
    HttpMethod.GET,
    "https://www.googleapis.com/calendar/v3/calendars/primary/events",
    { grantId },
  );
} catch (e) {
  if (e instanceof GrantExpiredError) {
    // Grant TTL expired — prompt user to reconnect
