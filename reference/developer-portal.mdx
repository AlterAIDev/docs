...
The Developer Portal ([portal.alterauth.com](https://portal.alterauth.com)) is where you configure OAuth providers, set security policies, and manage your application.
...
<Info>
**How is this different from OAuth?** With OAuth grants, the `grant_id` comes from your **end user** completing an OAuth flow via Alter Connect. With Managed Secrets, the `grant_id` comes from **this portal** when you store a credential. No end user is involved — you store the secret, get a `grant_id`, and use it in your code.
</Info>
...
<Steps>
  ...
  <Step title="Store a Secret">
    Click **Store Secret** on your provider, enter the credential value, and save. You'll receive a `grant_id` (UUID) to use with the SDK.
  </Step>
</Steps>
...
response = await vault.request(
    os.environ["LOYALTY_API_GRANT_ID"],
    HttpMethod.GET,
    "https://api.internal.com/v1/loyalty/points",
)
```
