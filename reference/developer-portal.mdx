...
<Note>
  Secret values are write-only — once stored, you cannot view the raw value. You can only use it via `alter_app.request()` or replace it by storing a new value.
</Note>

### Using Managed Secrets

Use the same `alter_app.request()` method as OAuth grants:
...
#
# The SDK's alter_app.request() uses a grant_id to look up
