...
|---|---|---|
| `dashboard_apps` | `read`, `write`, `delete` | [`apps`](/reference/cli/commands/apps) |
| `dashboard_keys` | `read`, `write`, `admin` | [`keys`](/reference/cli/commands/keys) |
| `dashboard_agents` | `read`, `write`, `admin` | [`agents`](/reference/cli/commands/agents) |
| `dashboard_providers` | `read`, `write`, `admin` | [`providers`](/reference/cli/commands/providers) |
| `dashboard_secrets` | `read`, `write`, `delete` | [`managed-secrets`](/reference/cli/commands/managed-secrets) |
| `dashboard_app_policy` | `read`, `rules_create`, `rules_update`, `rules_delete` | [`policy`](/reference/cli/commands/policy) — the `rules_*` verbs are mintable for API automation and consumed by `alter policy rules …` |
| `dashboard_audit` | `read` | [`audit`](/reference/cli/commands/audit) |
...
If a command returns [exit code 7 (`FORBIDDEN`)](/reference/cli/scripting#exit-codes), the PAT is valid but missing a scope — mint a new one with the broader set. Re-running `alter auth login` alone won't help, since it produces a token with the same scopes.
