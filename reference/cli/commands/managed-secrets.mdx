...
All commands are app-scoped — pass `--app <id>`, or [link a workspace](/reference/cli/scripting#workspace-linking) / set `ALTER_APP_ID`. Reads need `dashboard_secrets:read`; create / rotate / set-delegation-policy / grant operations need `dashboard_secrets:write`; `delete` needs `dashboard_secrets:delete`.
...
Set the user → agent [delegation](/concepts/delegation) policy on a managed secret — controls whether members of a group-typed grant may delegate the secret to an agent, and caps a delegation's lifetime. Needs `dashboard_secrets:write`.
