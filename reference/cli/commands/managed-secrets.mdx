...
The slug is **server-generated** as `<template-id>-<N>` (e.g. `discord-1`, `discord-2`) or `custom-<N>` for templateless secrets. Operators no longer pick a slug — the display name lives on `--name`, and the slug is just the stable machine handle the dashboard and audit logs reference. A stale request body that still ships a `slug` field is silently ignored.
