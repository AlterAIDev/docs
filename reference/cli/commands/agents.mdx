...
All commands are app-scoped — pass `--app <id>`, or [link a workspace](/reference/cli/scripting#workspace-linking) / set `ALTER_APP_ID`. Reads (`list` / `show` / `list-keys`) need `dashboard_agents:read`; `create` / `update` / `mint-key` need `dashboard_agents:write`; `revoke` and `revoke-key` need `dashboard_agents:admin`.
