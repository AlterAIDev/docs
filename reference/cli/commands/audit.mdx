...
Time filters: `--start <iso>` / `--end <iso>` give absolute bounds on every list-style command. The `--since <duration>` shorthand (e.g. `1h`, `7d`) — a relative start — is **only available on `audit list`**; `audit portal-actions`, `audit grant-events`, and `audit traces` accept `--start` / `--end` only. On `audit list`, `--start` and `--since` are mutually exclusive.
