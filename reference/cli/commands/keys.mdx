...
All commands are app-scoped — pass `--app <id>`, or [link a workspace](/reference/cli/scripting#workspace-linking) / set `ALTER_APP_ID`. `list` / `show` / `statistics` need `dashboard_keys:read`; `mint` / `rotate` / `rename` need `dashboard_keys:write`; `revoke` needs `dashboard_keys:admin`.
