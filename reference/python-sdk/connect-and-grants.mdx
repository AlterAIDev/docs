...
| `end_user_token` | `str \| None` | `None` | Scope the list to one end user by their JWT. Takes precedence over a configured `user_token_getter`. |
| `limit` | `int` | `100` | Page size, 1-1000. |
| `offset` | `int` | `0` | Page offset. |

When the `App` is constructed with a `user_token_getter`, the JWT is forwarded so the backend scopes the list to the resolving AppUser. Without a getter (and without `end_user_token`), the response is app-wide.
...
Returns a merged view: delegated OAuth grants the agent can act on AND agent-owned managed-secret grants. Without filters, the response is the complete view of grants visible to the calling agent.
