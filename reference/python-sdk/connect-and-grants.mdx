...
<Warning>
**One provider account uses one scope set.** Within a single application and provider, all agents that connect the **same end-user account** share one authorization. Requesting **different** `required_scopes` for different agents on that same account is **not supported** — the most recent connect determines the scopes for all of them, so an earlier agent can lose a scope it relied on. Use a **consistent** `required_scopes` set (or the Dev Portal default) per provider within an app. *This is a known limitation we may revisit.*
</Warning>
