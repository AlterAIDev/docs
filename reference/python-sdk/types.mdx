...
Returned by [`create_auth_session()`](/reference/python-sdk/connect-and-grants#create_auth_session). `__repr__` redacts `auth_url` (it carries the session token as the OIDC `state` parameter).
...
Returned by [`oauth_providers.list()`](/reference/python-sdk/connect-and-grants#oauth_providerslist).
...
A one-off deny rule passed to `with_constraints(rule=...)` — cryptographically
...
|---|---|---|
| `approval_id` | `UUID` | |
| `status` | `ApprovalStatusValue` | One of the seven status values (see [Connect & Grants](/reference/python-sdk/connect-and-grants#status-values)). |
| `expires_at` | `datetime` | |
...
Method `memory_scope() -> MemoryScope` derives the deterministic memory partition keys. See [Propagate identity into memory layers](/guides/propagate-identity-to-memory).
