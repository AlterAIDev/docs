...
- `auth_provider(*, base_url: str, providers: dict[str, list[str]] | None = None)` — construct an `AlterAuthProvider` for FastMCP (see [MCP](/reference/python-sdk/frameworks/mcp)). `base_url` is required (must be `https`, or a loopback host for local dev).
