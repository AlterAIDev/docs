...
- **`AmbiguousGrantError`** — surface the existing `account_identifiers` / `app_user_ids` fields to let the user pick, then retry with `account=` or `user_token=`. Minting a new session would be the wrong remediation here.
