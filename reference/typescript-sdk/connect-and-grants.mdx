...
- **App** — OAuth grants visible to the application. When `userTokenGetter` is configured (or `endUserToken` is passed), results are scoped to the resolved end user; otherwise app-wide grants are returned.
- **Agent** — every grant the agent can use: OAuth delegations plus agent-owned managed-secret grants. Mixed result type.

Both `AppListGrantsOptions` and `AgentListGrantsOptions` accept the same filters:
...
| `endUserToken` | `string` | — | Scope to one end user by their JWT. On `App`, takes precedence over `userTokenGetter`; on `Agent`, scopes the *delegated* OAuth grants (agent-owned managed-secret grants are unaffected). |
