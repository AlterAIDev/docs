...
When a request maps to one agent run (an LLM call, a tool invocation), construct the `Agent` inside the handler and dispose with `await using`:
...
The `await using` declaration calls `agent.close()` when the block exits, even on thrown errors.
...
Both `App` and `Agent` use the global `fetch` API and have no Node-only dependencies in the request path, so they run on the Edge runtime. The `Symbol.asyncDispose` lifecycle helper requires a runtime that implements it (Node 18+ with `--experimental-vm-modules`, or any runtime advertising TC39 explicit-resource-management support). Set `export const runtime = "edge"` on the route to opt in.
