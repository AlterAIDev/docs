...
Return a constrained sibling `Agent`. Pass `scopes` to narrow the scope set, `rule` to attach a one-off deny rule, or both; at least one is required. Calling it on a closed client throws [`AlterSDKError`](/reference/typescript-sdk/errors). See [`withConstraints` on the Client page](/reference/typescript-sdk/client#withconstraints) for the full surface, including the `RequestRule` shape.
