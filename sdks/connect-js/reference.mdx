...
await alterConnect.open({
  // Required
  token: sessionToken,            // From the backend
  onSuccess: (grants) => {},      // Called with array of Grant objects
...
`onSuccess` receives an array of `Grant` objects (multi-provider flow). Users can connect multiple providers in a single session.
...
await alterConnect.open({
  ...
  onSuccess: (grants) => {
    ...
    for (const grant of grants) {
      saveGrant(currentUser.id, grant.grant_id, grant.provider);
    }
  },
  onError: (error) => {
...
alterConnect.on('success', (grants) => {
  console.log('Connection successful:', grants);
});
