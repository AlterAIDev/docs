...
<Note>
  **Key Principle: Zero Token Exposure** - You never see or handle OAuth tokens. The SDK retrieves them from Alter Vault and injects them automatically into API calls. All token caching happens on the backend for performance (less than 10ms).
</Note>
...
- ✅ **Fast performance** - Backend Redis cache (less than 10ms response)
