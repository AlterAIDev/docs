...
Use `connection_policy` to control how long a connection stays active. When set, the Connect UI shows an expiry picker and the connection automatically expires after the chosen duration.

```python
# Restrict connections to a maximum of 7 days
results = await vault.connect(
    providers=["google"],
    connection_policy={
...
<Note>
  The expiry picker **only appears** when `connection_policy` is passed. Without it, connections have no automatic expiry. After a connection expires, token retrieval returns `ConnectionExpiredError` and the user must re-authorize via Alter Connect.
</Note>
...
async def safe_api_call(connection_id: str):
    async with AlterVault(
        ...
        try:
            ...
            # Connection expired, revoked, or deleted — user must re-authorize
            ...
            # Provider returned 403 and connection has outdated scopes
            print(f"Scope mismatch on {e.connection_id}: re-auth needed")
...
| `BackendError` | 401 | Invalid API key | Check your `api_key` |
