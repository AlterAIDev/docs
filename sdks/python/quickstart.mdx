...
await email_agent.request(
    gmail_connection_id, HttpMethod.GET,
    "https://gmail.googleapis.com/gmail/v1/users/me/messages",
)
await calendar_agent.request(
    calendar_connection_id, HttpMethod.GET,
    "https://www.googleapis.com/calendar/v3/calendars/primary/events",
)
...
<AccordionGroup>
  ...
  <Accordion title="Store and Use Connection IDs" icon="users">
    ...
    ```python
    # Look up connection_id from your database
    connection_id = get_connection_id_for_user(user_id, "google")
    await vault.request(connection_id, HttpMethod.GET, url)
    ```
  </Accordion>
