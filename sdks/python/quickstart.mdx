...
The same `vault.request()` method works for managed secrets (API keys, service tokens) stored via the Developer Portal. Just use the managed secret's `connection_id`:
