...
<Note>
  The expiry picker **only appears** when `connection_policy` is passed. Without it, connections have no automatic expiry. After a connection expires, token retrieval **raises** `ConnectionExpiredError` and the user must re-authorize via Alter Connect.
</Note>
