...
In user-facing applications — where many end users share a single `App` instance — configure `user_token_getter` so every grant operation is tied to the caller's identity. The SDK forwards the end user's IDP JWT and the backend resolves each call against that user's own grants.
...
FastAPI users get this wiring for free with [`AlterFastAPI`](/sdks/python/fastapi) — the dependency captures the incoming Bearer token and configures `user_token_getter` automatically.

If `user_token_getter` is configured but cannot produce a token (e.g. the end user's session has expired), the SDK raises `AlterSDKError`. Surface this as a 401 to the end user so they can re-authenticate.

Omit `user_token_getter` for headless tools (CLIs, cron jobs, backend migrations) that operate on grants the developer manages directly. The same methods then run under the application's own credentials.
...
The examples above use `App` (app key, `alter_key_app_*`). `App` is the application surface: it covers admin operations (managing grants, running Connect) *and* general application workloads where the backend acts under stored `grant_id`s. For **AI agent workloads** with their own identity-scoped access set, use the `Agent` class instead (agent key, `alter_key_agent_*`). The agent can only reach credentials explicitly bound to it — either user-delegated OAuth grants (`oauth_grant_agent_delegations` rows, written when a user completes Connect with `agent=<this-agent>`) or agent-owned managed secrets (operator-provisioned in the Developer Portal, no user in the loop). The agent never sees tokens or grant IDs — Alter resolves the right credential at the boundary.
...
response = await agent.request(
    ...
    "https://www.googleapis.com/calendar/v3/calendars/primary/events",
    provider="google",
    app_user_id="user_abc",      # operator's internal id for the end user
    user_token=end_user_jwt,     # signed JWT proving the end user authorized this call
)
```

If the end user has not delegated `google` access to this agent, `agent.request()` raises `NoDelegatedGrantError` (HTTP 404). Surface this to the application so it can run a Connect flow that includes `agent="<agent_id>"`.
...
# Hand session.connect_url to the end user.
```

### Agent self-Connect

If the agent itself triggers a Connect flow (e.g. the LangChain/MCP tool decorators do this automatically on `NoDelegatedGrantError`):

```python
session = await agent.create_connect_session(
    allowed_providers=["google"],
    app_user_id="user_abc",
    user_token=end_user_jwt,
)
```
...
Wrap a logical multi-call execution in `agent.trace(...)` to stamp every outgoing request with the same `run_id` (and optional `thread_id`/`metadata`) in the audit log:

```python
async with agent.trace(run_id="run_42", thread_id="thread_abc"):
...
|-----|--------|-----------------|
| Operator (admin) | `app.revoke_delegation(grant_id, agent_id)` | Any one delegation |
| Agent (self) | `agent.revoke_delegation(grant_id)` | The agent's own delegation for that grant |
...
await agent.request(HttpMethod.GET, url, provider="google", app_user_id="user_abc", user_token=jwt)
```
