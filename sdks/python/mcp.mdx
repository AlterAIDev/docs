...
alter_app = App(api_key="alter_key_...", caller="my-mcp-server")
...
- Sets an ambient audit context so any `alter_app.request()` call inside the tool body is automatically tagged with the tool name
...
The `AlterContext` object injected into the tool has a `.request()` method that works like `alter_app.request()` but with the provider pre-configured:
...
<Tip>
  For a step-by-step guide, see [Build a Multi-User MCP Server](/guides/mcp-claude-code).
</Tip>
...
alter_app = App(api_key="alter_key_...", caller="calendar-mcp")
alter = AlterMCP(alter_app)

auth = AlterAuthProvider(alter_app, base_url="http://localhost:8000/mcp", providers={"google": ["calendar.events"]})
