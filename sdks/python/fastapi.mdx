...
@app.post("/chat")
async def chat(alter_app=Depends(alter)):
...
@app.get("/connect")
async def connect(alter_app=Depends(alter)):
    """Create an Alter Connect session for OAuth authorization."""
    session = await alter_app.create_connect_session(
...
2. Returns the configured SDK client (`App` for app keys, `Agent` for agent keys — selected at construction time from the API-key prefix) with the token available for identity resolution

## How It Works

Under the hood, `AlterFastAPI` uses a task-scoped `ContextVar` to isolate tokens across concurrent requests. Each call to `Depends(alter)` captures the Bearer token for that specific request, so the SDK resolves the correct user identity when calling `alter_app.request(provider=...)`.
...
alter_app = App(api_key=..., user_token_getter=lambda: _token.get())
...
@app.post("/chat")
async def chat(alter_app=Depends(alter)):
    resp = await alter_app.request("GET", "https://api.example.com/v1/resource", provider="<provider>")
    return resp.json()
...
Both paths share the same SDK client and identity resolution — the REST path captures tokens via `Depends(alter)`, and the MCP path captures verified tokens through the auth provider callback.
...
| `**kwargs` | | Additional arguments passed to the constructed SDK client (`App` or `Agent`), except `user_token_getter` |

### `alter.vault`

The underlying SDK client (`App | Agent`). Use this when a
function needs the vault directly (e.g., `@alter_tool(alter.vault, provider=...)`).
For principal-specific access, `alter.app` and `alter.agent` raise
`AlterSDKError` when accessed against the wrong principal kind.
