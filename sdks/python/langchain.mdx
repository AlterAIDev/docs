...
alter_app = App(api_key="alter_key_...", caller="my-agent")
...
|-----------|------|---------|-------------|
| `vault` | `App \| Agent` | required | Alter Vault client instance (operator or workload — both expose `request()`) |
...
3. An ambient audit `ContextVar` is set with `{"tool": "<name>", "framework": "langchain", "run_id": "...", "thread_id": "..."}` so every `alter_app.request()` call inside the tool is automatically tagged
...
alter_app = App(api_key="alter_key_...", caller="email-agent")


@alter_tool(alter_app, provider="google", name="search_emails")
...
@alter_tool(alter_app, provider="github", name="list_repos")
