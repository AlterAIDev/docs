...
When only one provider is allowed, the SDK **automatically skips** the selection screen:

```javascript
// Backend: Only Google allowed
...
When multiple providers are allowed, users see a **provider selection grid**:
