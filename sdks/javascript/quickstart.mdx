...
<Note>
**OAuth connections only.** For Managed Secrets (API keys and service tokens), use the [Developer Portal](/reference/managed-secrets) instead.
</Note>
...
<Steps>
  <Step title="Backend creates session">
    The backend uses the API key to create a session token (no user_id or connection_id needed)
  </Step>
