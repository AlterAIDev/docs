...
<Note>
**Alter Connect is for OAuth connections only** — where your **end user** authorizes access to a third-party service (Google, Slack, GitHub). The `connection_id` is returned when the user completes the OAuth flow. For **Managed Secrets** (your own API keys/service tokens), use the [Developer Portal](/reference/managed-secrets) instead — no frontend SDK needed.
</Note>
