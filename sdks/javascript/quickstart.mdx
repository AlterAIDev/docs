...
<Note>
  **Important**: Store each `connection_id` in your database. You'll use these to retrieve OAuth tokens for API calls.
</Note>
...
const session = await vault.createConnectSession({
  endUser: { id: req.user.id, email: req.user.email },
