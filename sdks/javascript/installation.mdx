...
Alter Connect is a lightweight JavaScript SDK for embedding OAuth integrations into your application. Inspired by Plaid Link, the SDK opens a backend-served Connect UI in a popup -- the backend handles auth, provider selection, and OAuth, then sends results back via postMessage.

## What It Provides

- **Popup-based OAuth flow** - Opens backend-served Connect UI in a popup window
...
const ac = AlterConnect.create();
console.log(ac.getVersion()); // prints the installed version string
...
<script src="https://cdn.jsdelivr.net/npm/@alter-ai/connect@latest/dist/alter-connect.umd.js"></script>
<script>
  const ac = window.AlterConnect.create();
  console.log(ac.getVersion()); // prints the installed version string
</script>
```
