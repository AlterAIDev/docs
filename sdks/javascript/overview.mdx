...
<Note>
  **Important**: Store each `grant_id` in your database, mapped to your user. This is how you'll make API calls later via `vault.request(grant_id, ...)`. The `grant_id` is generated when your **end user** completes the OAuth flow — it's not something you create yourself.
</Note>
