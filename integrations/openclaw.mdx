...
Make an authenticated API call. This is the primary tool — the agent uses it to interact with external services.

| Parameter | Type | Description |
|-----------|------|-------------|
| `provider` | string | Provider ID: `"google"`, `"github"`, `"slack"`, `"microsoft"` |
...
| **Policy enforcement** | Every token retrieval is checked against your Cerbos policies (scopes, time-based access, rate limits). |
...
<AccordionGroup>
  ...
  <Accordion title="Agent gets 'Policy violation'" icon="shield">
    Your Cerbos policies are blocking the request. Check the policy configuration in the [Developer Portal](https://portal.alter.com) — common causes are scope restrictions, time-based access rules, or rate limits.
  </Accordion>
