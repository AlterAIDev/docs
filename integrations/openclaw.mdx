...
| **Policy enforcement** | Every token retrieval is checked against your Cerbos policies (time-based access, IP allowlist). |
...
<AccordionGroup>
  ...
  <Accordion title="Agent gets 'Policy violation'" icon="shield">
    Your Cerbos policies are blocking the request. Check the policy configuration in the [Developer Portal](https://portal.alterai.dev) under **App Details → Policies**. Common causes are scope restrictions, time-based access rules, or IP allowlist.
  </Accordion>
