...
The **Alter Vault OpenClaw plugin** lets your [OpenClaw](https://openclaw.dev) AI agent make authenticated API calls to external services (Gmail, GitHub, Slack, Sentry, and more) without ever exposing OAuth tokens to the LLM.
...
sequenceDiagram
    ...
    Alter Plugin->>Alter Vault API: Retrieve token (signed request)
    Alter Vault API-->>Alter Plugin: Token injected internally
...
1. An Alter Vault account with an application configured ([portal.alterai.dev](https://portal.alterai.dev))
...
<Steps>
  ...
  <Step title="Configure your Alter credentials">
    ...
    Get your API key and app ID from the [Developer Portal](https://portal.alterai.dev).
  </Step>
  ...
  <Step title="Connect a service">
    ...
    Agent: Here's a link to connect your Google account: https://connect.alterai.dev/session/...
...
| `connectionId` | No | Default connection ID (UUID) for API calls |
| `timeout` | No | Request timeout in milliseconds (default: `30000`) |

<Note>
  The SDK connects to the Alter API automatically. No URL configuration is needed.
</Note>

### Connection ID

The `connectionId` field specifies which OAuth connection to use. This is the UUID returned when a user completes the OAuth flow via Alter Connect.

<Warning>
  The `connectionId` must be a valid UUID belonging to your application. If the connection does not exist or has been revoked, API calls will result in `ConnectionNotFoundError`.
</Warning>
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `provider` | string | Provider ID: `"google"`, `"github"`, `"slack"`, `"atlassian"`, `"adobe"`, etc. |
...
Response: { connect_url: "https://connect.alterai.dev/session/abc123", expires_in: 1800 }

Agent: Here's a link to connect your Slack account:
       https://connect.alterai.dev/session/abc123
...
<AccordionGroup>
  ...
  <Accordion title="Sentry - List issues" icon="bug">
    ```
    alter_api(
      provider: "sentry",
      method: "GET",
      url: "https://sentry.io/api/0/projects/{org}/{project}/issues/",
      query_params: '{"query": "is:unresolved", "sort": "date"}'
    )
    ```
  </Accordion>
</AccordionGroup>
...
The plugin is built on the [Alter TypeScript SDK](/sdks/typescript/quickstart) (`@alter-ai/alter-sdk`) and inherits its **dual-client security pattern**:
...
| **Policy enforcement** | Every token retrieval is checked against your access policies (time-based access, IP allowlist). |
...
| `ReAuthRequiredError` | Connection expired/revoked/deleted | "Connection requires re-authorization. User needs to reconnect via alter_connect." |
...
<AccordionGroup>
  ...
  <Accordion title="Agent gets 'Connection not found' for a specific provider" icon="circle-question">
    Check that the `connectionId` in your plugin config is correct and the connection has not been revoked. You can verify connection status using `alter_connections`.
  </Accordion>

  <Accordion title="Agent gets 'Policy violation'" icon="shield">
    Your access policies are blocking the request. Check the policy configuration in the [Developer Portal](https://portal.alterai.dev) under **App Details → Policies**. Common causes are scope restrictions, time-based access rules, or IP allowlist.
  </Accordion>
