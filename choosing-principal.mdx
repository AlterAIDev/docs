...
└── No  → Is the action coming from a NAMED, OPERATOR-PROVISIONED workload?
         ...
         │               UUID comes from `app.agents.list()`.
...
| **SDK auth device** | `App` constructed with the app API key + user's JWT (via `user_token_getter`) | `App` constructed with the app API key | Either (a) `Agent` constructed with an agent API key (`alter_key_agent_…`) for workload isolation, or (b) `App(...).get_agent(agent_id)` returning an `Agent` whose underlying client carries `caller=<agent_uuid>`. Both produce the same backend identity. |
...
```python
vault = App(api_key=ALTER_API_KEY)   # NO user_token_getter
...
vault = Agent(api_key=AGENT_API_KEY)   # NO user_token_getter
...
#### Shape (b) — master key + `app.get_agent(uuid)` (use when monolithic)

```python
# Discover managed agent UUIDs once at boot. The list method returns
# an AgentListResult; iterate `.agents` for the items (the wrapper
# also exposes `total`, `limit`, `offset`, `has_more` for pagination).
app = App(api_key=APP_API_KEY)
result = await app.agents.list()
agents_by_name = {a.name: str(a.id) for a in result.agents}

# Construct a per-agent vault from the app key. `get_agent()` pins
# `caller=<agent_uuid>` on a fresh client and returns an Agent.
...
researcher_vault = app.get_agent(agents_by_name["researcher"])
...
Unlike shape (a), the underlying `caller=<uuid>` mechanism **accepts** a `user_token` alongside the agent UUID. When both are present the JWT-overrides rule applies: the user is the principal and owns the grant resolution; the agent UUID becomes audit attribution (not an access-boundary change). This is the "agent code path running inside a user session" pattern with full managed-agent attribution — for that pattern, construct an `App` with `caller=<uuid>` and a `user_token_getter` (NOT `app.get_agent()`, which is for autonomous-agent flows). See the next section.

There is **no** separate constructor option for the agent identity. All managed-agent UUIDs flow through the unified `caller=` parameter; the backend disambiguates free-form labels from managed-agent UUIDs server-side on each request. The `app.get_agent(uuid)` factory is a convenience wrapper around the same `caller=` mechanism for the autonomous-agent flow.
...
`app.agents.list()` returns an `AgentListResult` whose `.agents` field holds the managed agents provisioned for the calling app. Each item has `id` (UUID), `name` (human-readable label), `version`, and `status`. Typical pattern: list once at boot, build a `name → id` map from `result.agents`, then call `app.get_agent(uuid)` per agent (or, for the user-session pattern below, construct an `App` with `caller=` set to the matching UUID). The list method itself works with the app API key (no agent key needed); it's a discovery endpoint that returns metadata only — no secrets. Use `result.has_more` + `offset` to page through additional agents if the directory is large enough to span pages.
...
boot_app = App(api_key=APP_KEY)
agent = await boot_app.agents.get_by_name("email-research-bot")
if agent is None:
...
| **Agent** | Operator creates an agent on the dashboard. The agent gets a stable `id` (UUID) and a per-agent API key (shown once at creation; optional to use). Operator binds managed secrets to the agent on the agent detail page; each binding produces a `grant_id`. | Two equivalent runtime shapes — pick based on isolation needs: **(a)** the agent reads its per-agent API key from env / config and constructs `Agent(api_key=AGENT_KEY)`; or **(b)** the application uses the app API key, calls `app.agents.list()` once at boot to discover the agent's `id`, and calls `app.get_agent(id)` to obtain an `Agent` for that agent — then calls `vault.request(grant_id=…)`. |
