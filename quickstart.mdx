...
<Info>
**The key distinction**: For OAuth, your **end user** triggers the grant by logging into their account (Google, Slack, etc.), and the `grant_id` is returned to you. For Managed Secrets, **you** (the developer) store the credential in the portal and get the `grant_id` back. Once you have a `grant_id`, usage is identical: `vault.request(grant_id, ...)`.
</Info>
        ...
        await alterConnect.open({
            ...
            onSuccess: (connections) => {
                ...
                for (const conn of connections) {
                    console.log('Connected!', conn.grant_id, conn.provider_id);
                }
                loadIntegrations();
            },
            onError: (error) => {
...
async with AlterVault(
    ...
    # POST with JSON body
    response = await vault.request(
        slack_grant_id,  # from Alter Connect
        HttpMethod.POST,
    ...
    response = await vault.request(
        sentry_grant_id,  # from Alter Connect
        HttpMethod.GET,
...
<Info>
**How is this different from OAuth?** With OAuth, the `grant_id` comes from your **end user** completing an OAuth login. With Managed Secrets, the `grant_id` comes from **you** (the developer) storing a credential in the Developer Portal. No end-user action is needed.
</Info>
...
5. **Copy the `grant_id` returned** -this is what you'll use in your code
...
### Zero Token Exposure
`vault.request()` **is your HTTP client** -it replaces direct `fetch` or `httpx` calls. Instead of constructing auth headers yourself, you pass a `grant_id` and the target URL. The SDK:
...
- **OAuth grants**: The `grant_id` is returned in the `onSuccess` callback when your **end user** completes the OAuth flow via Alter Connect. Store this in your database, mapped to your users.
- **Managed secrets**: The `grant_id` is returned by the **Developer Portal** when you store a credential. Hard-code it in your config or environment variables.
...
await vault.request(os.environ["LOYALTY_API_GRANT_ID"], HttpMethod.GET, url)
```
...
### Provider Support
The SDK works with **any** OAuth provider configured in your dashboard. The first parameter is always the `grant_id`:

```python
# Use the grant_id from Alter Connect
await vault.request(google_grant_id, HttpMethod.GET, url)   # from Alter Connect
await vault.request(github_grant_id, HttpMethod.GET, url)   # from Alter Connect
await vault.request(slack_grant_id, HttpMethod.POST, url)   # from Alter Connect
```
...
### Grant Management
- Store the `grant_id` from Alter Connect in your database
- Map grant IDs to your users for easy lookup
...
- The `grant_id` is correct and belongs to your app
