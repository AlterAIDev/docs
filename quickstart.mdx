...
|--------|------------|--------------|
| **Headless** (`vault.connect()`) | CLI tools, scripts, Jupyter notebooks, backend agents. No frontend at all. | SDK creates the session, opens the user's default browser, and polls until OAuth completes. Everything happens in Python/TypeScript. |
| **Redirect** (`connect_url`) | Server-rendered apps (Django, Rails, etc.), email links, or any app that can redirect a browser. No JavaScript SDK needed. | Your backend creates a session, gets a `connect_url`, and redirects the user to it. After OAuth, the user is redirected back to your `return_url` with the results. |
| **Popup** (`@alter-ai/connect` SDK) | Single-page apps (React, Vue, etc.) where you want a popup overlay with real-time `onSuccess`/`onError` callbacks without leaving the page. | Your backend creates a session and passes the `session_token` to the frontend SDK, which opens the hosted page in a popup. |
...
- Community: [Join our Community](mailto:founders@alterai.dev)
