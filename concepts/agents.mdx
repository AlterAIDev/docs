...
- **User-delegated OAuth grants** — a user consented to delegate one of their connections to this agent during the OAuth flow. See [Delegation](/concepts/credentials#delegation).
- **Managed-secret grants it owns or that are delegated to it** — an operator issued a managed-secret grant directly to this agent, or a user delegated one to it. See [Delegation](/concepts/credentials#delegation).
...
- [Delegation](/concepts/credentials#delegation) — sharing a user's grant with an agent.
