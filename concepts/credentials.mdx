...
<Note>
**Group principals are not supported by every identity provider.** A group grant requires the IDP to deliver real-time group lifecycle webhooks so a removed member's access is revoked immediately. Today that means Clerk, and Okta with stable-ID group keying — group grants cannot be created against Auth0 or Custom OIDC providers. See the [per-provider support table](/admin/identity-provider#how-directory-sync-works).
</Note>
