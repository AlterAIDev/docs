---
title: 'Scopes'
description: "How a credential's reach is narrowed from what it could do to what this call needs."
---
...
- `connect:initiate` — mint Connect sessions.
- `tokens:retrieve` — retrieve credentials for direct API calls.
- `proxy:execute` — execute provider calls through the proxy surface.
- `spans:emit` — emit user-defined telemetry spans.
- `identity:resolve` — resolve verified identity contexts (see [Propagate identity to memory](/guides/propagate-identity-to-memory)).
- `identity:assert` — mint signed identity assertions.
...
- [Connections](/concepts/credentials#oauth-grants) — where provider scopes are first established.
